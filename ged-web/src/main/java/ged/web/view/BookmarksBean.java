...
public class BookmarksBean extends AbstractBean {
	...
	public void add(final AbstractAuditedEntity entity) {
		BookmarksBean.logger.debug("Adding bookmark {} for user {}", entity, this.sessionBean.get().getEmail());
		Bookmark bookmark = this.createBookmark(entity);
		if (this.bookmarks.size() > 9) {
			BookmarksBean.logger.warn("Bookmark full for user {}", this.sessionBean.get().getEmail());
	...
	private Bookmark createBookmark(final AbstractAuditedEntity entity) {
		...
		bookmark.setUser(this.sessionBean.get());
		return bookmark;
	}

	private void findAllBookmarks() {
		this.bookmarks = this.bookmarkService.findAllByUser(this.sessionBean.get());
	}
