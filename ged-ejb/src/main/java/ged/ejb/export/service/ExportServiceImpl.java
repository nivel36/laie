...
@Stateless
public class ExportServiceImpl extends AbstractService<Export> implements ExportService {
	...
	@Inject
	public ExportServiceImpl(@Repository ExportDao exportDao) {
	...
	public void saveDefinition(final ExportSaveDefinitionInputBean newDefinition) {
		...
		for (SaveDefinitionItem item: newDefinition.getList()) {
	...
	private static Set<Long> toExportDefinitionSet(final Set<ExportDefinition> source) {
		...
		for (ExportDefinition item: source) {
